...
class TestSettingsDefaults:
    ...
    def test_default_reazonspeech_repo_dir(self):
        ...
        assert s.reazonspeech_repo_dir.replace("\\", "/").endswith(".vendor/ReazonSpeech")

    def test_default_irodori_repo_dir(self):
        s = Settings(_env_file=None)
        assert Path(s.irodori_repo_dir).is_absolute()
        assert s.irodori_repo_dir.replace("\\", "/").endswith(".vendor/Irodori-TTS")


class TestSettingsFromEnv:
    def test_irodori_repo_dir_from_env(self):
        with patch.dict(os.environ, {"IRODORI_REPO_DIR": ".vendor/custom-irodori"}):
            s = Settings(_env_file=None)
            assert Path(s.irodori_repo_dir).is_absolute()
            assert s.irodori_repo_dir.replace("\\", "/").endswith(".vendor/custom-irodori")
    ...
    def test_aivis_settings_from_env(self):
        with patch.dict(
            ...
            assert s.aivis_engine_dir.replace("\\", "/").endswith(".vendor/aivis")
    ...
    def test_reazonspeech_repo_dir_from_env(self):
        with patch.dict(os.environ, {"REAZONSPEECH_REPO_DIR": ".vendor/reazon"}):
            s = Settings(_env_file=None)
            assert Path(s.reazonspeech_repo_dir).is_absolute()
            assert s.reazonspeech_repo_dir.replace("\\", "/").endswith(".vendor/reazon")

    def test_irodori_server_dir_default(self):
        s = Settings(_env_file=None)
        assert Path(s.irodori_server_dir).is_absolute()
        assert s.irodori_server_dir.replace("\\", "/").endswith(".vendor/Irodori-TTS-Server")
